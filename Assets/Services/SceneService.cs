...
public class SceneService
{
    ...
    public List<ObjectData> LoadSceneObjects(string filePath)
    {
        ...
        foreach (string line in lines)
        {
            ...
            {
                translation = new Vector3(float.Parse(values[0]), float.Parse(values[1]), float.Parse(values[2])),
                rotation = new Vector3(float.Parse(values[3]), float.Parse(values[4]), float.Parse(values[5])),
                scale = new Vector3(float.Parse(values[6]), float.Parse(values[7]), float.Parse(values[8]))
            };
